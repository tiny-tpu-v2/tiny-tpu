...
# original leaky_relu_child test data (4 values total)
INPUT_DATA_VALUES = [2.5, -1.2, 0.8, -3.1]  # mix of positive and negative values

@cocotb.test()
async def test_leaky_relu_parent_4x2_staggered(dut):
    ...
    if use_col1 and col1_idx < len(BATCH_4x2_DATA_COL1):
        dut.lr_data_1_in.value = to_fixed(BATCH_4x2_DATA_COL1[col1_idx])
    ...
    if use_col2 and col2_idx < len(BATCH_4x2_DATA_COL2):
        dut.lr_data_2_in.value = to_fixed(BATCH_4x2_DATA_COL2[col2_idx])
    ...
    # collect outputs - leaky_relu outputs appear one cycle after input (registered)
    col1_results = []
    col2_results = []
    
    for cycle_num in range(10):
        if dut.lr_valid_1_out.value.integer:
            output_val = from_fixed(dut.lr_data_1_out.value.integer)
            col1_results.append(output_val)
            print(f"cycle {cycle_num}: col1 output = {output_val:.5f}")
            
        if dut.lr_valid_2_out.value.integer:
            output_val = from_fixed(dut.lr_data_2_out.value.integer)
    ...
    expected_col1 = [compute_leaky_relu(data, LEAK_FACTOR) for data in BATCH_4x2_DATA_COL1]
    ...
    expected_col2 = [compute_leaky_relu(data, LEAK_FACTOR) for data in BATCH_4x2_DATA_COL2]
...
@cocotb.test()
async def test_leaky_relu_parent_as_single_child(dut):
    ...
    print("=== test case 2: leaky_relu_parent as single leaky_relu_child (4 values) ===")
    print(f"leak factor: {LEAK_FACTOR}")
    
    # set leak factor
    dut.lr_leak_factor_in.value = to_fixed(LEAK_FACTOR)
    
    # feed original leaky_relu_child test data through column 1 only - unrolled
    # sample 0
    idx = 0
    input_data_val = INPUT_DATA_VALUES[idx]
    dut.lr_data_1_in.value = to_fixed(input_data_val)
    dut.lr_valid_1_in.value = 1
    dut.lr_valid_2_in.value = 0  # column 2 unused
    dut.lr_data_2_in.value = 0
    ...
    # compute expected results: leaky_relu(data, leak_factor)
    expected_results = [compute_leaky_relu(data, LEAK_FACTOR) for data in INPUT_DATA_VALUES]
...
@cocotb.test()
async def test_leaky_relu_parent_edge_cases(dut):
    ...
    print("=== test case 3: leaky_relu_parent edge cases ===")
    ...
    dut.lr_data_1_in.value = to_fixed(input_val)
    dut.lr_valid_1_in.value = 1
    dut.lr_valid_2_in.value = 0
    ...
    for cycle_num in range(6):
        if dut.lr_valid_1_out.value.integer:
            output_val = from_fixed(dut.lr_data_1_out.value.integer)
            col1_results.append(output_val)
            expected = compute_leaky_relu(edge_cases[len(col1_results)-1], LEAK_FACTOR)
...
@cocotb.test()
async def test_leaky_relu_parent_invalid_inputs(dut):
    ...
    print("=== test case 4: leaky_relu_parent invalid input cases ===")
    
    # set leak factor and data
    dut.lr_leak_factor_in.value = to_fixed(LEAK_FACTOR)
    dut.lr_data_1_in.value = to_fixed(1.5)
    dut.lr_data_2_in.value = to_fixed(-2.0)
    ...
    await RisingEdge(dut.clk)
    # assert dut.lr_valid_1_out.value.integer == 0, "col1 output should be invalid when lr_valid_1_in is low"
    # assert dut.lr_valid_2_out.value.integer == 0, "col2 output should be invalid when lr_valid_2_in is low"
    # assert dut.lr_data_1_out.value.integer == 0, "col1 output data should be zero when invalid"
    # assert dut.lr_data_2_out.value.integer == 0, "col2 output data should be zero when invalid"
    ...
    await RisingEdge(dut.clk)
    # assert dut.lr_valid_1_out.value.integer == 1, "col1 output should be valid when lr_valid_1_in is high"
    # assert dut.lr_valid_2_out.value.integer == 0, "col2 output should be invalid when lr_valid_2_in is low"
    ...
    await RisingEdge(dut.clk)
    # assert dut.lr_valid_1_out.value.integer == 0, "col1 output should be invalid when lr_valid_1_in is low"
    # assert dut.lr_valid_2_out.value.integer == 1, "col2 output should be valid when lr_valid_2_in is high"
    ...
    print("all leaky_relu_parent tests completed successfully!")
