...
async def test_pe(dut):
    ...
    dut.pe_weight_in.value = to_fixed(2.0) # in next cycle, gets latched in background buffer of pe11!
    await RisingEdge(dut.clk)
    ...
    dut.pe_weight_in.value = to_fixed(1.0)
    await RisingEdge(dut.clk)
    ...
    dut.pe_input_in.value = to_fixed(5.0)
    dut.pe_psum_in.value = to_fixed(0.0) 
    await RisingEdge(dut.clk)
