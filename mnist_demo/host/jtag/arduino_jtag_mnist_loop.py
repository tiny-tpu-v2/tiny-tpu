...
from mnist_demo.mnist_tools import FRAME_BYTES
from mnist_demo.mnist_tools import FRAME_HEADER
...
def run_mmio_command(args: argparse.Namespace, mmio_args: list[str]) -> tuple[int, str, str]:
    command = [
        "bash",
        str(PROJECT_DIR / "host" / "jtag" / "run_system_console_mmio.sh"),
        *mmio_args,
    ]
...
class PersistentSystemConsole:
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        self._tcl_script_win = to_windows_path(PROJECT_DIR / "host" / "jtag" / "mnist_jtag_mmio.tcl")
...
def maybe_program_fpga(args: argparse.Namespace) -> None:
    if not args.auto_program:
        return
    command = ["bash", str(PROJECT_DIR / "fpga" / "program_fpga_jtag.sh")]
