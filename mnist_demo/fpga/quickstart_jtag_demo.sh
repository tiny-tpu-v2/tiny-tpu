...
HOST_DIR="$PROJECT_DIR/host/jtag"
RUNTIME_DIR="$PROJECT_DIR/artifacts/runtime/jtag_host"
cd "$PROJECT_DIR"
...
prepare_sample_bits_file() {
    local output_file="$RUNTIME_DIR/sample_image_0.bits"
    mkdir -p "$RUNTIME_DIR"
...
sample_bin = Path("data/model/reference/sample_image_0.bin")
...
run_health_check() {
    local output
    output="$(bash "$HOST_DIR/run_system_console_mmio.sh" health 2>&1)" || {
        printf '%s\n' "$output"
        return 1
    }
    printf '%s\n' "$output"
...
run_one_shot_predict() {
    ...
    bash "$HOST_DIR/run_system_console_mmio.sh" \
...
exec python3 -u "$HOST_DIR/arduino_jtag_mnist_loop.py" "${LOOP_ARGS[@]}"
